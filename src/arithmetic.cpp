...
ArithmeticCalculator::~ArithmeticCalculator() { variables.clear(); }

bool ArithmeticCalculator::isValidExpression(const std::string &expression,
...
bool ArithmeticCalculator::checkExpression(const std::string &expression,
                                           size_t *errorPos) const {
  if (expression.empty()) {
    if (errorPos)
      *errorPos = 0;
...
ArithmeticCalculator::tokenize(const string &expression) const {
...
bool ArithmeticCalculator::isUnaryMinus(const vector<string> &tokens,
                                        size_t index) const {
  if (tokens[index] != "-")
    return false;
  if (index == 0)
...
vector<string>
ArithmeticCalculator::toRPN(const std::string &expression) const {
...
double ArithmeticCalculator::applyOperator(double a, double b,
...
double ArithmeticCalculator::applyFunction(const string &func,
                                           double arg) const {
  if (func == "sin")
    return sin(arg);
  else if (func == "cos")
    ...
    if (arg < -1.0 || arg > 1.0)
      throw invalid_argument("asin argument out of range [-1, 1]");
    return asin(arg);
  } else if (func == "acos") {
    if (arg < -1.0 || arg > 1.0)
      throw invalid_argument("acos argument out of range [-1, 1]");
    return acos(arg);
  } else if (func == "atan")
    return atan(arg);
  else if (func == "sqrt") {
    ...
    if (arg <= 0)
      throw invalid_argument("log of non-positive number");
    return log(arg);
  } else if (func == "log10") {
    if (arg <= 0)
      throw invalid_argument("log10 of non-positive number");
    return log10(arg);
  } else if (func == "exp")
    return exp(arg);
  else if (func == "abs")
...
bool ArithmeticCalculator::isFunction(const string &str) {
  return str == "sin" || str == "cos" || str == "tan" || str == "asin" ||
         str == "acos" || str == "atan" || str == "sqrt" || str == "log" ||
         str == "log10" || str == "exp" || str == "abs" || str == "floor" ||
         str == "ceil" || str == "round";
}
