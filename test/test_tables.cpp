...
TEST(HashTableTest, LinearProbingWorks) {
    ...
    table.insert("A", Polinom::parseFromString("x + y"));
    table.insert("E", Polinom::parseFromString("2x + 2y")); // Может вызвать коллизию с A
...
TEST(HashTableTest, RehashOnFull) {
    ...
    table.insert("C", Polinom::parseFromString("3x + 3y")); // Должен вызвать rehash
...
TEST(EdgeCasesTest, VeryLongKey) {
    ...
    string longKey = "very_very_long_key_name_that_should_work_fine_" + string(100, 'a');
    Polinom p = Polinom::parseFromString("x + y");
