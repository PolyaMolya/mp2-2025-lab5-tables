...
private:
  ...
  std::shared_ptr<RBNode<KeyType, ValueType>>
  findNode(const KeyType &key) const {
    auto cur = root;
    while (cur) {
      comparisons++;
      if (key < cur->key)
  ...
  void inorder(std::shared_ptr<RBNode<KeyType, ValueType>> node,
  ...
  std::shared_ptr<RBNode<KeyType, ValueType>>
  minimum(std::shared_ptr<RBNode<KeyType, ValueType>> node) {
...
public:
  ...
  size_t getOperations() const override { return operations; }
};
